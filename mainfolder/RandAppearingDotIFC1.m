...
trialMat34=[sort(repmat(1:2,1,4), 'descend') zeros(1,8)];
...
strc2step=10; strc2Rev=0; corResp2=0; strc2UD=[]; strc2STEPS=[];
...
while trialNo<length(trialMat)
    ...
    if trialMat(2,trialNo)==0
        ...
        randloc=trialMat(1,trialNo);
        fprintf('SameLocDot, %d. column.\n', randloc)
        scndloc=randloc;
        commm2=['echo iod.file: C:\\i686-w64-mingw32\\tmp\\thresholdtests\\2IFCRandomLocDot\\' num2str(randloc) '-' num2str(scndloc) '-dot.txt>client.in'];
        system(commm2, '-echo');
        WaitSecs(4.05);
        system(commm2, '-echo');
        ...
        elseif trialMat(3,trialNo)==2 %staircase2
            if rndloc-strc2step>2
                ...
                scndloc=rndloc+strc2step; %right
            end
            staircase=2;
            fprintf('DiffLocDot, %d and %d. column. Staircase %d. \n', randloc, scndloc, staircase)
                    commm2=['echo iod.file: C:\\i686-w64-mingw32\\tmp\\thresholdtests\\2IFCRandomLocDot\\' num2str(randloc) '-' num2str(scndloc) '-dot.txt>client.in'];
    ...
    fprintf(output, '\n%d\t %d\t %d\t %d\t %d\t %d', trialNo, staircase, randloc, scndloc, acc, spacing);
    
    if acc
        if trialMat(2, trialNo)==1
            if corResp1>2
                if strc1step>1
