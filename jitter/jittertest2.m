...
system(commm1,'-echo');
commm3='echo iod.file: C:\\i686-w64-mingw32\\tmp\\jittersstatics2\DR450S.txt >client.in';
randomizer=randperm(24);
names=aji2.names;
duration=aji2.duration;
condition=aji2.condition;
orientation=aji2.ori;
trlcntr=1;
accuracy=0;
...
for repeat=1:5
    Shuffler=Shuffle(1:24);
    names=names(:,Shuffler);
    condition=condition(:,Shuffler);
    orientation=orientation(:,Shuffler);
for i=1:length(orientation)
    ...
    filename=names{rnd};
            commm2=['echo iod.file: C:\\i686-w64-mingw32\\tmp\\jittersstatics2\' filename '>client.in'];
